    ...
    cfdi(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                var iva = 0;
                const { id } = req.params;
                const pedidod = yield Pedidodetalle.aggregate([
                    { "$lookup": {
                ...
                fs_1.default.writeFile('./src/CSD_emisor/certificado.cer', base64file_cer, { encoding: 'base64' }, function (err) {
                    console.log('.cer creado');
                });
                let base64key = configuracion[0]['key'];
                let base64file_key = base64key.split(';base64,').pop();
                fs_1.default.writeFile('./src/CSD_emisor/key.key', base64file_key, { encoding: 'base64' }, function (err) {
                ...
                var date = (0, moment_timezone_1.default)(fecha).format('YYYY-MM-DDThh:mm:ss');
                // const fecha =moment.tz('America/Mexico_City').format('YYYY-MM-DDThh:mm:ss'); 
                const facturas = yield index_1.factura.find().count();
                ...
                // fin del for
                const impuesto = new core_1.Impuestos({ TotalImpuestosTrasladados: iva.toFixed(2).toString() });
                impuesto.traslados({
                    Base: SubTotal,
                    Impuesto: impuesto_material,
                    TipoFactor: tipo_factor,
                ...
                soap.createClient(url, function (err, client) {
                    client.WSTimbrado.BasicHttpsBinding_IWSTimbrado.TimbrarObjeto(args, function (err, result) {
                        if (err) {
            ...
            const generarpdf = (xml) => __awaiter(this, void 0, void 0, function* () {
                let factura = '';
                ...
                let qr = yield QRCode.toDataURL(qrdatos);
                return new Promise((resolve) => {
                    var options = [];
                    for (const valor in data) {
                        options.push(data[valor]['_attributes']);
                    }
                    const tableRows = options === null || options === void 0 ? void 0 : options.map((row) => `<tr><td>${row['NoIdentificacion']}</td><td>${row['ClaveUnidad']}</td><td>${row['Descripcion']}</td><td align="right">${row['ValorUnitario']}</td><td align="right">${row['Cantidad']}</td>
      ...
      <style>
        ...
        `;
                    pdf.create(table).toBuffer(function (err, buffer) {
                        if (err) {
