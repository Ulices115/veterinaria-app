...
router.post('/', [
    ...
    check('id_producto').custom(db_validators_1.default.existeinventarioprod),
    // check('inventarios').custom(validaciones.existeinventarioprod),
    validar_campos_1.default.validarCampos,
], inventario_1.default.crearinventario);
router.get('/', [], inventario_1.default.obtenerinventarios);
router.get('/:id', [
    //  check('id','No es un id de mongo valido').isMongoId(),
    validar_campos_1.default.validarCampos
], inventario_1.default.obtenerinventario);
router.put('/:id', [
    // validarjwt.validarJWT,
    check('id_producto', 'El id producto es obligatorio').not().isEmpty(),
    validar_campos_1.default.validarCampos
], inventario_1.default.actualizarinventario);
router.delete('/:id', [
    //validarjwt.validarJWT,
    check('id', 'No es un id de mongo valido').isMongoId(),
    check('id').custom(db_validators_1.default.existeinventario),
    validar_campos_1.default.validarCampos
], inventario_1.default.borrarinventario);
