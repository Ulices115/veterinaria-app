...
export class transacciones{
    creartransaccion = async ( req:any,res:Response) => {
        ...
        const transaccionDB = await Transaccion.findOne({transaccion});
        if(transaccionDB){
            return res.status(400).json({
                msg: `la transaccion ${transaccion} ya existe`
            })
        } 
    ...
    obtenertransaccion = async(req:any,res:Response ) => {
        const { id } = req.params;
        const trans = await Transaccion.find({activo:true,'transaccion': {'$regex': id,'$options': 'i'}}).sort({descripcion:1}).limit(10);
        res.json( trans );
